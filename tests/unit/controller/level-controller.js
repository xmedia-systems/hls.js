...
describe.only('LevelController', () => {
  ...
  it('should trigger level switch when level is manually set', () => {
    ...
    assert(triggerSpy.calledWith(Event.LEVEL_SWITCHING, {
      bitrate: 246440,
  ...
  describe('onManifestLoaded handler', function () {
    ...
    it('should trigger hlsManifestParsed when levels are found in the manifest', () => {
      ...
      assert(triggerSpy.calledWith(Event.MANIFEST_PARSED, {
        altAudio: false,
        audio: false,
        audioTracks: [],
        firstLevel: 0,
    ...
    it('should signal altAudio if present in the manifest', function () {
      let data = {
        audioTracks: [
